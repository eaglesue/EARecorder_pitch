...
真实的流程就是这样，当启动每个unit后，每个unit都在等待获取数据，于是可以在回调函数中调用AudioUnitRender，来向上一级unit申请数据（上一级unit的回调函数就会响应），如果上一级也没有，再向上一级申请。当第一级unit处理好数据后，就在自己的回调函数中把音频数据放在指定参数上，这样下一级unit就可以继续处理了。
